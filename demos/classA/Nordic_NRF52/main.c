...
/**
 * @brief Size of the buffer used to receive data for the application.
 */
#define LORAWAN_RECEIVE_BUFFER_SIZE    ( 32 )
...
static void prvLorawanClassATask( void * params )
{
    ...
    uint8_t buffer[ LORAWAN_RECEIVE_BUFFER_SIZE ];
    size_t recvLength;
    uint16_t recvPort;
    ...
    if( status == LORAMAC_STATUS_OK )
    {
        ...
        status = LoRaWAN_Join( LORAWAN_JOIN_TYPE_OTAA, LORAWAN_DEFAULT_DATARATE );
    }
          ...
          char ** argv )
{
    ...
    /* Add user tasks */
    xTaskCreate( prvLorawanClassATask, "LoRaWanClassA", 2048, NULL, tskIDLE_PRIORITY, NULL );
